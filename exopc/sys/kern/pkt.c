...
xoknet_init (void)
{
  ...
  bzero (si->si_networks, (XOKNET_MAXNETS * sizeof (struct network)));
...
{
  int i = si->si_nnetworks;
  struct network *xoknet = &(si->si_networks[i]);
...
{
  struct network *xoknet = &(si->si_networks[cardno]);
  int segcnt;
  int ret;
  int i, j;
  ...
  if ((cardno < 0) || (cardno >= si->si_nnetworks))
    {
...
sys_nettap (u_int sn, u_int k, int id, u_int interfaces)
{
  ...
  for (i = 0; i < XOKNET_MAXNETS; i++)
    {
	 ...
	 ((r = acl_access (&c, &(si->si_networks[i].netcap), 1,
			   ACL_ALL)) < 0)))
	{
	  MP_SPINLOCK_RELEASE (GLOCK(NETTAP_LOCK));
	  return r;
  ...
  if (si->si_num_nettaps >= MAX_NETTAPS)
    {
      MP_SPINLOCK_RELEASE (GLOCK(NETTAP_LOCK));
      return -E_NO_NETTAPS;
    }
