...
{
  Ppage_pp_klock_acquire(pp);
  bzero (pp->pp_acl, sizeof(pp->pp_acl));
  pp->pp_buf = NULL;
  pp->pp_status = type;
  vpagel_init(&(pp->pp_vpl));
  pp->pp_refcnt = 0;
  pp->pp_pcfc = 0;
  pp->pp_readers = 0;
  pp->pp_writers = 0;
  pp->pp_state.ps_writers = PP_ACCESS_ALL;
  pp->pp_state.ps_readers = PP_ACCESS_ALL;
  Ppage_pp_klock_release(pp);
}
...
#if 0
static void
translate (Pde * pdir, u_int la) 
{
  int i;
  Pde *pdep = &pdir[PDENO (la)];
  Pde pde   = pdir[PDENO (la)];
  Pte *pt   = ptov (pde & ~PGMASK);
  Pte *ptep = &pt[PTENO (la)];
  Pte pte   = pt[PTENO (la)];

  printf ("la %08x\n", la);
  printf ("  ==> PDE ADDR %08x\n", (uint32)pdep - KERNBASE);
  printf ("  ==> PDE %08x\n", pde);
  printf ("  ==> PTE ADDR %08x\n", (uint32)ptep - KERNBASE);
  printf ("  ==> PTE %08x\n", pte);
...
i386_vm_init ()
{
  ...
  extern unsigned int cpuid_features;
  int i;
  uint32 cr0;

  freemem = PGROUNDUP((uint32)&end);  // every thing past BSS is free

  //////////////////////////////////////////////////////////////////////
  // create initial page directory.
  bzero ((void *)freemem, NBPG);
  p0pdir_boot = (void *)freemem;
  p0cr3_boot = (uint32)kva2pa(p0pdir_boot);
  freemem += NBPG;

  //////////////////////////////////////////////////////////////////////
  // Map the kernel stack:
  //   [KSTACKTOP-NBPD, KSTACKTOP]  -- the complete VA range of the stack
  //     * [KSTACKTOP-KSTKSIZE, KSTACKTOP] -- backed by phys memory
  //     * [KSTACKTOP-NBPD, KSTACKTOP-KSTKSIZE] -- not backed => faults

  // allocate page table for the stack
  bzero ((void *)freemem, NBPG);
  kstkmap = (void *)freemem;
  freemem += NBPG;

  // phys pages are in the data section at bootstacktop
  assert ((uint32)&bootstacktop % NBPG == 0);
  assert (KSTKSIZE % NBPG == 0);
  for (i = 0; i < KSTKSIZE/NBPG; i++)
    ((Pte *)kstkmap)[NLPG - i + 1] = kva2pa(&bootstacktop + i*NBPG)|PG_P|PG_W;

  // map the stack into VA at KSTACKTOP-NBPD
  // XXX PG_U really?
  p0pdir_boot[PDENO(KSTACKTOP-NBPD)] = kva2pa(kstkmap)|PG_P|PG_W|PG_U; 

  
  //////////////////////////////////////////////////////////////////////
  // All of physical memory is mapped at KERNBASE. 
  // Ie.  the VA range [KERBASE, 2^32 - 1] should map to
  //      the PA range [0, 2^32 - 1 - KERBASE]   
  // We might not have that many (ie. 2^32 - 1 - KERBASE)    
  ...
  if (cpuid_features & 0x8) {
    ...
    lcr4 (rcr4() | CR4_PSE);
    for (i = 0; i < NPPT; i++)
      p0pdir_boot[PDENO(KERNBASE) + i] = (i*NBPD)|PG_P|PG_W|PG_PS;
  } else {
    ...
    for (i = 0; i < NPPT; i++) {
      p0pdir_boot[PDENO(KERNBASE) + i] = kva2pa(freemem)|PG_P|PG_W;
      freemem += NBPG;
    }
  }
  ...
  p0pdir_boot[PDENO(VPT)]  = (Pde)kva2pa(p0pdir_boot)|PG_P|PG_W;
  // same fore UVPT
  // XXX PG_U??
  p0pdir_boot[PDENO(UVPT)] = (Pde)kva2pa(p0pdir_boot)|PG_P|PG_W|PG_U;



  //////////////////////////////////////////////////////////////////////
  // On and x86 segmementation maps a VA to a LA (linear addr) and
  ...
  p0pdir_boot[0] = p0pdir_boot[PDENO(KERNBASE)];
  ...
  // Current mapping: KERNBASE+x => x => x. (paging uses p0pdir_boot[0])

  // reload all segments.  And setup the IDT. 
  asm volatile ("lidt _idt_pd+2"); // XXX use a macro
  asm volatile ("lgdt _gdt_pd+2"); // XXX use a macro
  asm volatile ("movl %%ax,%%gs" :: "a" (GD_UD|3));
  asm volatile ("movl %%ax,%%fs" :: "a" (GD_UD|3));
  asm volatile ("movl %%ax,%%es" :: "a" (GD_KD));
  asm volatile ("movl %%ax,%%ds" :: "a" (GD_KD));
  asm volatile ("movl %%ax,%%ss" :: "a" (GD_KD));
...
ppage_init (void) __XOK_NOSYNC
{
  ...
  freemem = PGROUNDUP(freemem);                 /* Not actually needed */
  ...
  MP_SPINLOCK_INIT(Ppage_pp_klock_ptr(&ppages[i]));
  ppages[i++].pp_status = PP_INVAL;

  /* save the second page for loadkern so we can reboot on
   *  top of ourselves */

  MP_SPINLOCK_INIT(Ppage_pp_klock_ptr(&ppages[i]));
  ppages[i++].pp_status = PP_IOMEM;
  ...
  for (lim = (min (basememsize, IOPHYSMEM) >> PGSHIFT); i < lim; i++)
    {
      ppage_initpp (&ppages[i], PP_FREE);
      ppages[i].pp_pinned = 0;
      Ppagelist_head_insert (&free_list, &ppages[i]);
...
#else /* SMP */
  {
	...
	ap_boot_addr_base = pp2pa (&ppages[i]) + NBPG; 

	for (lim = (min (basememsize, IOPHYSMEM) >> PGSHIFT); i < lim; i++)
	  ppages[i].pp_status = PP_KERNEL;
      }
  }
#endif
  ...
  __XOK_SYNC(locks pp->klock)
{
  ...
  Ppage_pp_klock_acquire(pp);
  ppage_initpp (pp, pp->pp_status);
  pp->pp_acl[0].c_valid = 1;
  pp->pp_acl[0].c_perm = CL_ALL;
  Ppage_pp_klock_release(pp);
}
             ...
             calls bc_is_dirty; bc_set_clean; bc_remove)
{
  ...
  if (bc_is_dirty (pp->pp_buf))
    bc_set_clean (pp->pp_buf);
	     ...
	     releases PPAGE_FBUF_LOCK)
{
  ...
  n = si->si_nfreepages - si->si_nfreebufpages;
  ...
  *pp = NULL;			/* XXX -- just to be safe */
  ...
  printf("%d, %d, %d\n", si->si_nppages, si->si_ndpages, si->si_nfreepages);
  return -E_NO_MEM;
}
  ...
  while (*pp)
	...
	*pp = (&((*pp)->pp_link))->le_next;
	continue;
      ...
      for (i = 0; i < n; i++)
	{
	  if ((p >= &ppages[nppage]) || (!ppage_is_reclaimable (p))) 
	  {
  ...
  __XOK_SYNC(locks ppage->pp_klock; calls ppage_free)
{
  ...
  if (*ptep & PG_P)
  {
    ...
    va &= ~PGMASK;
    for (vp = vpagel_get_first(&(pp->pp_vpl)); vp; 
...
ppage_acl_check (struct Ppage *pp, cap *c, int len, u_char perm)
  __XOK_REQ_SYNC(on pp->pp_klock)
{
  return acl_access(c, pp->pp_acl, len, perm);
}
	     ...
	     calls ppage_free)
{
  ...
  for (num = *num_completed; num < total; num++, va += NBPG)
      ...
      if (new_pg)
	{
	  ...
	  if (acl_add (pp->pp_acl, PP_ACL_LEN, &c, &r)) 
	  {
	    ppage_free(pp);
	    Ppage_pp_klock_release(pp);
	    return r;
	...
	return r;
      ...
      else if (!ppcompat_state (pp->pp_readers + 1, 
	                        pte&PG_W ? pp->pp_writers + 1 : pp->pp_writers,
				&(pp->pp_state)))
      {
        Ppage_pp_klock_release(pp);
	return -E_SHARE;
	     ...
	     releases ppage->pp_klock)
{
  ...
  for (; num; num--, va += NBPG)
	...
	adj = 1;
      ...
      if (!ppcompat_state (pp->pp_readers, pp->pp_writers + adj,
	                   &(pp->pp_state)))
      {
	*ptep = orig_pte;
	Ppage_pp_klock_release(pp);
  ...
  __XOK_SYNC(locks pp->pp_klock)
{
  ...
  ps->ps_readers = (r_state == -1 ? pp->pp_state.ps_readers : r_state);
  ps->ps_writers = (w_state == -1 ? pp->pp_state.ps_writers : w_state);
  
  if (!ppcompat_state (pp->pp_readers, pp->pp_writers, ps))
  {
    free(ps);
    Ppage_pp_klock_release(pp);
    return -E_SHARE;
  ...
  __XOK_SYNC(locks pp->pp_klock; calls acl_read)
{
  ...
  Ppage_pp_klock_acquire(pp);
  r = acl_read (pp->pp_acl, PP_ACL_LEN, res, res_len);
  Ppage_pp_klock_release(pp);
  ...
  __XOK_SYNC(locks pp->pp_klock; calls ppage_acl_check; calls acl_setsize)
{
  ...
  r = acl_setsize (pp->pp_acl, PP_ACL_LEN, size);
  Ppage_pp_klock_release(pp);
  ...
  __XOK_SYNC(locks pp->pp_klock; calls acl_mod)
{
  ...
  Ppage_pp_klock_acquire(pp);
  r = acl_mod (&c, pp->pp_acl, PP_ACL_LEN, pos, n);
  Ppage_pp_klock_release(pp);
...
repeat:
      ...
      eid = vpage_get_env(vpagel_get_first(&(pp->pp_vpl)));
      Ppage_pp_klock_release(pp);
