		 ...
		 struct Pp_state *pp_state_user)
{
  ...
  copyin (pp_state_user, pp_state_kernel, sizeof(struct Pp_state));

  /* check the page level permissions */
  if ((r = env_getcap (curenv, k, &c)) < 0)
  {
    free(pp_state_kernel);
    return r;
  }
  ...
  pp = &ppages[ppn];
  if (pp->pp_status != PP_USER)
  {
    free(pp_state_kernel);
    return -E_INVAL;
  }

  if (!ppcompat_state (pp->pp_state.ps_readers, pp->pp_state.ps_writers,
	               pp_state_kernel))
  {
    free(pp_state_kernel);
    return -E_SHARE;
		   ...
		   u_int va, u_int ke, int envid)
{
  ...
  if (xn_kernel.xa_dev != pp->pp_buf->buf_dev) return -E_INVAL;

  if (xn_kernel.xa_dev >= MAX_DISKS) {
    xtnt.xtnt_block = pp->pp_buf->buf_blk;
    xtnt.xtnt_size = NBPG;
  } else {
    xtnt.xtnt_block = pp->pp_buf->buf_blk *
  ...
  if ((r = ppage_insert (e, pp, va, pte & PGMASK)) < 0) {
    if (pp->pp_refcnt == 0) ppage_free (pp);
    return r;
  }
...
bc_flush_by_buf (struct bc_entry *buf)
{
  ...
  pp = &ppages[buf->buf_ppn];
  assert (pp->pp_status == PP_FREE || pp->pp_status == PP_USER);

  /* don't flush a mapped buffer */
  /* don't flush a buffer for a pinned page*/
  /* don't flush a dirty buffer */
  if (pp->pp_status == PP_USER || pp->pp_pinned || buf->buf_dirty)
    return;
...
sys_bc_ppn2buf64 (u_int sn, u32 ppn, u32 *d, u32 *b_hi, u32 *b_low)
{
  ...
  if (pp->pp_buf == NULL) return -E_NOT_FOUND;

  /* else copy the dev and blk number and return 0 */
  copyout(&(pp->pp_buf)->buf_dev, d, sizeof(*d));
  blk_hi = QUAD2INT_HIGH((pp->pp_buf)->buf_blk64);
  blk_low = QUAD2INT_LOW((pp->pp_buf)->buf_blk64);
		   ...
		   u_int envid)
{
  ...
  struct bc_entry *bc_entry = pp->pp_buf;
#ifdef XN
