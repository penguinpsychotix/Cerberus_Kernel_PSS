...
{
  assert (dev < si->si_ndisks);
  return si->si_disks[dev].d_bsize;
}
...
disk_giveaway (void)
{
  ...
  for (i = 0; i < si->si_ndisks; i++) {
    struct disk* d = &(si->si_disks[i]);
    ...
    si->si_pxn[i].xa_dev = i;
    si->si_pxn[i].xa_name = 0;
    ret = pxn_alloc (1, 1, 1, &cap, &cap, &(si->si_pxn[i]));
    if (ret != 0) {
      /* bootup */
    ...
    ret = pxn_kernel_add_xtnt (&(si->si_pxn[i]), &xtnt);
    if (ret != 0) {
      /* bootup */
...
{
  if ((bp == NULL) || (bp->b_dev >= si->si_ndisks)) {
  ...
  /* call appropriate strategy routine */
  si->si_disks[bp->b_dev].d_strategy (bp);
...
disk_pollforintrs (u_int devno)
{
  ...
  assert (si->si_scsictlr_cnt == 1);
  return (ncr_intr (si->si_scsictlr_irqs[0]));
}
