...
static struct Partition *load_partition_table (int dev, int *error) {
  ...
  pt = (struct Partition *) 
    ((char *)pp2va (&ppages[bc->buf_ppn]) + PAR_TBL_START);
  return pt;
}
...
int partition_init () {
  ...
  if (next_partition != 0) {
    bcopy (new_si_disks, si->si_disks, sizeof(struct disk)*MAX_DISKS);
    si->si_ndisks=next_partition;
  }
