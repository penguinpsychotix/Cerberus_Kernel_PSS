...
{
  uint32 highest;
  uint32 ef = read_eflags ();
  write_eflags (ef ^ FL_ID);
  if (ef != read_eflags()) {
    // only when eflags can be twiddled..
    cpuid (0, &highest, NULL, NULL, NULL);
    if (highest >= 1) 
      cpuid (1, &cpuid_vers, NULL, NULL, &cpuid_features);
  }
  write_eflags (ef); // restore
}
...
i386_init (void)
{
  ...
  printf ("%s\n", osid); // XXX haven't yet called osid_init
