...
sched_init (void) 
{
  ...
  for (j = 0; j < get_cpu_count(); j++)
    {
      si->si_percpu_ticks[j] = 0;
      si->si_percpu_idle_ticks[j] = 0;
  ...
  __XOK_NOSYNC
{
  ...
  to_new->to_ticks = si->si_system_ticks + ticks;
  to_new->to_func = func;
  to_new->to_arg = arg;
  ...
  __XOK_SYNC(calls revoke_process; calls env_free; calls sched_runnext)
{
  ...
  si->si_percpu_ticks[cpu]++;
  if (curenv == env0)
    si->si_percpu_idle_ticks[cpu]++;
  ...
  while ((to_list[cpu]) && 
         (to_list[cpu]->to_ticks <= si->si_system_ticks))
  {
